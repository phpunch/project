...
data_dir=/media/punch/DriveD/D_Download/LibriSpeech
trials=exp/trials
nnet_dir=exp/xvector_nnet_1a
musan_root=/media/punch/DriveD/D_Download/musan
stage=0



if [ $stage -le 0 ]; then
 echo "Prepare Data for LibriSpeech"
 # Prepare Data for LibriSpeech
 #for name in train-clean train-clean-360 dev-clean test-clean; do
 for name in dev-clean test-clean; do
   ./data_prep.sh ${data_dir}/${name} data/${name}
 done
 utils/combine_data.sh data/train data/train-clean data/train-clean-360 data/dev-clean

fi

if [ $stage -le 1 ]; then
 echo "#############################"
 echo "### START MAKE MFCC & VAD ###"
 echo "#############################"
 # Make MFCCs and compute the energy-based VAD for each dataset
 for name in train test-clean; do
...
if [ $stage -le 2 ]; then
  echo "Augment the LibriSpeech data with reverberation"
  frame_shift=0.01
...
if [ $stage -le 3 ]; then
  echo "Take a random subset of the augmentations"
  # Take a random subset of the augmentations
  utils/subset_data_dir.sh data/train_aug 500000 data/train_aug_1m
  utils/fix_data_dir.sh data/train_aug_1m

  echo "Make MFCCs for the augmented data"
  ...
  echo "Combine the clean and augmented LibriSpeech list"
...
if [ $stage -le 4 ]; then
  echo "we prepare the features to generate examples for xvector training. (CMVN + remove nonspeech frame)"
...
if [ $stage -le 5 ]; then
  echo "Remove features that are too short after removing silence"
  ...
  utils/filter_scp.pl data/train_combined_no_sil/utt2num_frames data/train_combined_no_sil/utt2spk > data/train_combined_no_sil/utt2spk.new #filter unwanted utt
  mv data/train_combined_no_sil/utt2spk.new data/train_combined_no_sil/utt2spk
  utils/fix_data_dir.sh data/train_combined_no_sil

  echo "Throw out speakers with fewer than 8 utterances"
...
# # Copy pretrained model to exp
# cp -r download_model/$nnet_dir/ $nnet_dir

### Extract xvector

if [ $stage -le 9 ]; then
 echo "##############################"
 echo "### Start Extract x-vector ###"
 echo "##############################"
 ...
 sid/nnet3/xvector/extract_xvectors.sh --cmd "$train_cmd --mem 4G" --nj 40 \
   $nnet_dir data/test-clean \
   $nnet_dir/xvectors_test-clean
 echo "##############################"
 echo "### End Extract x-vector   ###"
 echo "##############################"
fi
...
if [ $stage -le 11 ]; then
 ...
 echo "Start"  
 $train_cmd exp/scores/log/test-clean_scoring.log \
   ivector-plda-scoring --normalize-length=true \
   "ivector-copy-plda --smoothing=0.0 $nnet_dir/xvectors_train/plda - |" \
   "ark:ivector-subtract-global-mean $nnet_dir/xvectors_train/mean.vec scp:$nnet_dir/xvectors_test-clean/spk_xvector.scp ark:- | transform-vec $nnet_dir/xvectors_train/transform.mat ark:- ark:- | ivector-normalize-length ark:- ark:- |" \
   "ark:ivector-subtract-global-mean $nnet_dir/xvectors_train/mean.vec scp:$nnet_dir/xvectors_test-clean/xvector.scp ark:- | transform-vec $nnet_dir/xvectors_train/transform.mat ark:- ark:- | ivector-normalize-length ark:- ark:- |" \
   "cat '$trials' | cut -d\  --fields=1,2 |" exp/scores_test-clean || exit 1;
 echo "End"

fi

if [ $stage -le 12 ]; then
  eer=`compute-eer <(local/prepare_for_eer.py $trials exp/scores_test-clean) 2> ./out.txt`
  mindcf1=`sid/compute_min_dcf.py --p-target 0.01 exp/scores_test-clean $trials 2> /dev/null`
  mindcf2=`sid/compute_min_dcf.py --p-target 0.001 exp/scores_test-clean $trials 2> /dev/null`
  echo "EER: $eer%"
